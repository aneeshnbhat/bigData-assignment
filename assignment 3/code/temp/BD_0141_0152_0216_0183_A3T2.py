...
occurrences = df3.where((df3.word == givenWord) & (df3.recognized==False)).groupby('countrycode').count().withColumnRenamed("count","cnt").filter("cnt <" +str(k)).orderBy(df3.countrycode.asc()).collect()
