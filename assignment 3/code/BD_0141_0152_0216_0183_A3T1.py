...
a = (df.where((df.word == givenWord) & (df.recognized==True)).select(F.avg(df.Total_Strokes)).collect()[0][0])
b = (df.where((df.word == givenWord) & (df.recognized==False)).select(F.avg(df.Total_Strokes)).collect()[0][0])
print(round(a,5))
print(round(b,5))
